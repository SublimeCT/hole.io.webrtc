...
| 移动方式           | 方向键/WASD 或鼠标/指针拖拽方向，无路径寻路                                                                                                                                                        | ✅ 已实现 |
...
| 玩家初始出生       | 由对局 seed 在地图内随机生成；三名玩家间距至少为初始直径加 `12m` 安全距离，禁止重叠                                                                                                                | ✅ 已实现 |
...
- HUD：✅ 比赛显示玩家分数、尺寸等级、半径、下一级进度、剩余时间和包括人类玩家的三人实时排名；屏幕边缘显示带圆弧尾部的 Bot 方向/距离 SVG 箭头；吞噬时播放 transform/opacity `+分值` 动画和合成吞噬音效，移动设备触发短震动；结束后显示完整三人最终排名，`R` 可重新开始。
