...
- HUD：✅ 比赛显示玩家分数、尺寸等级、半径、下一级进度、剩余时间和包括人类玩家的三人实时排名；技能卡仅在对局中显示，以整块背景填充表示冷却并显示激活/引信/充能状态；屏幕边缘显示带圆弧尾部的 Bot 方向/距离 SVG 箭头；吞噬时播放 transform/opacity `+分值` 动画和合成吞噬音效，移动设备触发短震动；对局中和结算界面均可返回主页；结束后显示完整三人最终排名，`R` 可重新开始；`Ctrl+R` / `Command+R` 始终由浏览器刷新页面，不触发技能或重开。
