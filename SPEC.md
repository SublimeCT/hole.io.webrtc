...
| 黑洞视觉         | stencil 切除与地表齐平的圆形区域；地表下为等半径透明黑色直筒与同色底面，不参与物理碰撞，直筒深度随洞半径等比增长。地面圆样式（`HoleRenderer`）：一圈静态纯色细环勾边（用户所选颜色），外侧成长弧从 12 点钟方向顺时针填充、升级时重置并触发加色闪光；技能态含 Q 加速拖尾、E 范围涟漪、R 引信红弧（贴洞缘）与爆炸闪光、重生无敌盾牌沿洞缘公转；领先黑洞上方显示皇冠（随半径等比）。每帧显式清空颜色/深度/stencil 缓冲 | ✅ 已实现 |
